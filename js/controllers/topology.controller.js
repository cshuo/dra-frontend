...
.controller('topologyCtrl', [
		...
		function ($scope, $http, $location, $timeout, $interval, $websocket, VmsMap) {
			...
			//     {type: "status", id: "host_id1", target:"host", status: "healthy"}];
			...
			$scope.initWS = function () {
				...
				var connect_interval = $interval(function(){
					if($scope.ws.socket.readyState != 1){   //not open yet
						console.log("Try to connect to websocket Server...");
						$scope.ws = $websocket('ws://20.0.1.10:8070/soc');
						onmsg();
					} else {
						console.log("connected to the websocket server...")
					$interval.cancel(connect_interval);
					}
				}, 10000);

				var onmsg = function () {
					$scope.ws.onMessage(function(message){
						var updateArr = JSON.parse(message.data).msg;
						...
						updateArr.forEach(function (updateData) {
							if(updateData.type === "reset"){
								...
								if(updateData.warning !== undefined){
									...
									$scope.warning_data[updateData.id] = {};
									$scope.warning_data[updateData.id].type = updateData.target;
								}
								if((updateData.status === "normal" || updateData.status === "healthy") && $scope.warning_data[updateData.id] !== undefined){
									delete $scope.warning_data[updateData.id];
								}
